 ...
 * 检测token是否携带，没有携带直接写response响应
 * 携带的话组装CustomUsernamePasswordToken提交给安全管理器，再到用户认证器，
 * 用户认证器(CustomHashedCredentialsMatcher)从自定义域(CustomRealm)里
 * 拿出token信息进行验证
 * @Author: 小霍
 * @UpdateUser: 小霍
 * @Version: 0.0.1
...
public class CustomAccessControlFilter extends AccessControlFilter {
    ...
    protected boolean onAccessDenied(ServletRequest servletRequest, ServletResponse servletResponse) throws Exception {
        ...
        try {
            ...
            // 提交验证器认证
           super.getSubject(servletRequest,servletResponse).login(customUsernamePasswordToken);
