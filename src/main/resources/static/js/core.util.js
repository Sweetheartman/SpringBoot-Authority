    ...
    coreUtil.sendAjax = function (url, params, ft, method,headers, noAuthorityFt, async,contentType) {
        ...
        layui.jquery.ajax({
            ...
            beforeSend: function(request) {
                // 如果请求的接口是用户登录接口，需要在头部携带验证码id和用户输入的验证码值
                if(url == "/api/user/login"){
                    request.setRequestHeader("captcha-id", CoreUtil.getData("captcha-id"));
                    request.setRequestHeader("captcha-text", CoreUtil.getData("captcha-text"));
                }
